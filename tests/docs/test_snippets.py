...
SNIPPETS_DIR = Path(__file__).parent.parent.parent / "docs" / "src" / "snippets"
DOCS_DIR = Path(__file__).parent.parent.parent / "docs" / "src" / "side"
SDK_DIR = Path(__file__).parent.parent.parent / "docs" / "src" / "sdk-reference"
...
def handle_cdp(
    ...
    replaced = dedent("""
        ...
        notte = NotteClient()
        first_session = notte.Session()
        first_session.start()
        cdp_url = first_session.cdp_url()
    """).strip()

    code = code.replace(initial, replaced)
    code += "\nfirst_session.stop()"
