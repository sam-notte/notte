...
This method validates the agent creation request and sets up the appropriate connections with the provided vault and session if specified


## Parameters

<ParamField path="session" type="RemoteSession">
  The session to connect to.
</ParamField>

<ParamField path="vault" type="UnionType[NotteVault, None]">
  A notte vault instance, if the agent requires authentication (default: None)
</ParamField>

<ParamField path="notifier" type="UnionType[BaseNotifier, None]">
...
[`RemoteAgent`](../misc/remoteagent): A new RemoteAgent instance configured with the specified parameters.
