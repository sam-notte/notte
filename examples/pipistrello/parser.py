...
class PipistrelloParser(BaseParser):
...
"""
    ...
    def parse(self, text: str) -> NotteStepAgentOutput | None:
        url = self.search_pattern(text, PipistrelloParser.observe_tag)
        action = self.search_pattern(text, PipistrelloParser.step_tag)
        scrape = self.search_pattern(text, PipistrelloParser.scrape_tag)
        output = self.search_pattern(text, PipistrelloParser.done_tag)
        match (bool(url), bool(action), bool(scrape), bool(output)):
            case (True, False, False, False):
                return NotteStepAgentOutput(
                    observe=GotoAction.model_validate(self.parse_json(text, PipistrelloParser.observe_tag))
                )
            case (False, True, False, False):
                return NotteStepAgentOutput(
                    step=ExecutableAction.model_validate(self.parse_json(text, PipistrelloParser.step_tag)),
                )
            case (False, False, True, False):
                return NotteStepAgentOutput(
                    scrape=ScrapeAction.model_validate(self.parse_json(text, PipistrelloParser.scrape_tag))
                )
            case (False, False, False, True):
                return NotteStepAgentOutput(
                    completion=CompletionAction.model_validate(self.parse_json(text, PipistrelloParser.done_tag))
                )
            case _:
                return None
